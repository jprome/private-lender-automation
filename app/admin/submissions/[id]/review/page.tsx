    ...
    return (
      <main style={{ maxWidth: 960, margin: '32px auto', padding: 16 }}>
        ...
        <p style={{ marginTop: 12, color: '#b00020' }}>
          Missing INJECTED_EMAIL (or RELAY_EMAIL) env. Cannot build preview payload.
        </p>
      </main>
    );
  }
  ...
  return (
    <main style={{ maxWidth: 960, margin: '32px auto', padding: 16 }}>
      ...
      <div style={{ marginTop: 12, color: '#444' }}>
        <div>
          <strong>ID:</strong>{' '}
          <span style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas' }}>{data.id}</span>
        </div>
        <div>
          <strong>Created:</strong> {data.created_at ? new Date(data.created_at).toLocaleString() : ''}
        </div>
        <div>
      ...
      <section style={{ marginTop: 18 }}>
        <h2 style={{ fontSize: 18, fontWeight: 700 }}>PrivateLender Payload Preview</h2>
