  ...
  return (
    <main style={{ maxWidth: 960, margin: '32px auto', padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12 }}>
        ...
        <Link prefetch={false} href="/admin/submissions">← Back</Link>
      </div>

      <div style={{ marginTop: 12, color: '#444' }}>
        <div><strong>ID:</strong> <span style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas' }}>{data.id}</span></div>
        <div><strong>Created:</strong> {data.created_at ? new Date(data.created_at).toLocaleString() : ''}</div>
        <div><strong>Status:</strong> {data.status}</div>
        <div><strong>Email (sent to lender):</strong> {injectedEmail}</div>
        <div><strong>User email (stored):</strong> {data.email}</div>
